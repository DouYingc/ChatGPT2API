...
def has_image_tool_arguments_json(message: str) -> bool:
    text = str(message or "").strip()
    ...
    try:
        ...
        return False
    return isinstance(data, dict) and "size" in data and "n" in data
...
def is_model_text_reply_instead_of_image(message: str) -> bool:
    ...
    if REFERENCED_IMAGE_IDS_RE.search(message):
        return True
    if has_image_tool_arguments_json(message):
        return True
...
IMAGE_RESOLUTION_HINTS = {
    "1k": "目标输出分辨率为 1K 级别，优先保证构图稳定和生成成功。",
...
def build_image_prompt_with_options(prompt: str, size: str | None, quality: str = "auto", resolution: str | None = None) -> str:
    final_prompt = build_image_prompt(prompt, size, quality).strip()
...
def image_plan_candidates(request: "ConversationRequest") -> list[str | None]:
    ...
    requested_plan = normalize_image_plan_type(request.plan_type)
    model_plan = normalize_image_plan_type(model_plan_type)
    if allowed:
        if requested_plan:
            return [requested_plan] if requested_plan in allowed else []
        if model_plan:
            return [model_plan] if model_plan in allowed else []
        return list(allowed)
    if requested_plan:
...
def image_generation_model_for_tool(model: str) -> str:
    ...
    if base_model in {"gpt-image-2", "codex-gpt-image-2"}:
        return "gpt-image-2"
    return "gpt-image-2"
...
def conversation_events(
    ...
    final_prompt = prompt_with_global_system(build_image_prompt_with_options(prompt, size, quality, resolution)) if image_model else prompt
...
def stream_image_outputs(
    ...
    if message and not file_ids and not sediment_ids and last.get("blocked"):
        ...
        return
    turn_use_case = str(last.get("turn_use_case") or "").lower()
    should_poll_for_image = (
        is_supported_image_model(request.model)
        or bool(request.images)
        or turn_use_case == "image gen"
        or (last.get("tool_invoked") is True and "image" in turn_use_case)
    )
    ...
    poll_timeout = poll_timeout_secs or config.image_poll_timeout_secs
...
def _codex_response_error(value: Any) -> tuple[str, str, str] | None:
    if isinstance(value, dict):
        error = value.get("error")
        if isinstance(error, dict):
            message = str(error.get("message") or "").strip()
            code = str(error.get("code") or "upstream_error").strip() or "upstream_error"
            error_type = str(error.get("type") or "server_error").strip() or "server_error"
            if message:
                return f"Codex upstream error ({code}): {message}", error_type, code
        for item in value.values():
            found = _codex_response_error(item)
            if found:
                return found
...
def stream_codex_image_outputs(
    ...
    if hasattr(backend, "generate_codex_image"):
        events = backend.generate_codex_image(
            prompt=build_image_prompt(request.prompt, request.size, request.quality),
            image_size=image_size or codex_image_size_for_request(request) or str(request.size or ""),
    ...
    if not images:
        upstream_error = _codex_response_error(events)
        if upstream_error:
            message, error_type, code = upstream_error
            raise ImageGenerationError(
                message,
                status_code=503 if code in {"server_is_overloaded", "service_unavailable"} else 502,
                error_type=error_type,
                code=code,
            )
        raise ImageGenerationError("No image result found in Codex response")
    data = format_image_result(
    ...
    if data:
        ...
        return
    raise ImageGenerationError("Codex returned image data, but it could not be saved")
...
def _generate_single_image(
    ...
    while True:
        try:
            ...
            if not candidates:
                raise RuntimeError("no available image account plan")
        ...
        codex_image_size = codex_image_size_for_request(request)
        try:
            ...
            if codex_image_size:
                output_iter = stream_codex_image_outputs(backend, request, codex_image_size, index, total)
            elif is_codex_image_model(request.model):
                output_iter = stream_codex_image_outputs(backend, request, None, index, total)
            else:
                output_iter = stream_image_outputs(
                    backend,
                    request,
                    index,
                    total,
            ...
            if not returned_result:
                ...
                if request.deadline_at is not None and time.monotonic() >= request.deadline_at:
                    raise ImageGenerationError("图片生成等待超过可用时间，请重试", account_email=account_email)
            ...
            # Match the reference flow: once the upstream request has started,
            # do not abandon that generation and start over with another account.
            ...
            if codex_image_size:
                resolution = normalize_image_resolution(request.resolution) or ""
                raise ImageGenerationError(
                    f"{resolution.upper()} 高清生成失败，未降级为普通清晰度。原因：{image_stream_error_message(error_text)}",
            ...
            if not emitted_for_token and is_rate_limit_error(exc):
                account_service.mark_image_rate_limited(
                    token,
                    error=last_error,
                    headers=getattr(exc, "headers", None),
            ...
            if codex_image_size:
                resolution = normalize_image_resolution(request.resolution) or ""
                raise ImageGenerationError(
                    f"{resolution.upper()} 高清生成失败，未降级为普通清晰度。原因：{image_stream_error_message(last_error)}",
                    code="high_resolution_generation_failed",
                    account_email=account_email,
                    conversation_id="",
                ) from exc
