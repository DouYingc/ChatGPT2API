...
class OpenAIBackendAPI:
    ...
    def iter_codex_image_response_events(
        ...
        try:
            self._report_progress("requesting_codex")
            proxy_url = str(config.get_proxy_settings() or "").strip()
            opener = (
            ...
            logger.info({
                "event": "codex_responses_urllib_proxy",
                "proxy": proxy_url,
            })
            with opener.open(request, timeout=1200) as raw:
    ...
    def _start_image_generation(self, prompt: str, requirements: ChatRequirements, conduit_token: str, model: str,
        ...
        sse_timeout = max(30.0, min(120.0, float(config.image_poll_timeout_secs) / 2))
        response = self.session.post(
            self.base_url + path,
    ...
    def _iter_image_sse_payloads_with_idle_timeout(self, response: requests.Response) -> Iterator[str]:
        try:
            idle_timeout = max(20.0, min(60.0, float(config.image_poll_timeout_secs) / 3))
        except Exception:
            idle_timeout = 40.0
        ...
        threading.Thread(target=reader, name="image-sse-reader", daemon=True).start()
        yielded_payload = False
        while True:
            try:
                kind, value = events.get(timeout=idle_timeout)
            except queue.Empty:
                logger.warning({
                    "event": "image_sse_idle_timeout",
                    "idle_timeout_secs": idle_timeout,
                    "yielded_payload": yielded_payload,
                })
                response.close()
                return
            if kind == "payload":
            ...
            if kind == "error":
                logger.warning({
                    "event": "image_sse_read_error",
                    "yielded_payload": yielded_payload,
                    "error": repr(value)[:300],
                })
                if yielded_payload:
                    return
                text = str(value).lower()
                if "timeout" in text or "curl: (28)" in text or "curl: (56)" in text or "closed" in text:
                    return
                raise value
            return
    ...
    def _stream_picture_conversation(
        ...
        try:
            yield from self._iter_image_sse_payloads_with_idle_timeout(response)
        finally:
            response.close()
